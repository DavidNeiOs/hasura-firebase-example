...
export const Home: React.FC<HomeProps> = ({}) => {
  ...
  const { loading: fetchLoading, data } = useQuery(GET_USER, {
    variables: { id },
  });
  const [
    updateUser,
    { loading: mutationLoading, data: response },
    ...
    <SafeAreaView style={tailwind('flex-1')}>
      <View style={tailwind('py-10 px-5')}>
        <View style={tailwind('flex-row justify-between items-center')}>
          ...
          <TouchableOpacity
            style={tailwind(
              'rounded-full bg-red-500 h-10 w-10 items-center justify-center'
            )}
        ...
        <View style={tailwind('mt-10')}>
          ...
          {(!name || editing) && (
            <View>
              ...
              <View
                style={tailwind(
                  'border-b border-blue-500 mt-2 flex-row justify-between items-center pb-2'
                )}
              >
                <TextInput
                ...
                <TouchableOpacity
                  style={tailwind(
                    'bg-blue-500 h-8 w-8 rounded-full items-center justify-center'
                  )}
                  onPress={() => {
                    updateUser({
                      variables: {
          ...
          {name && !editing && (
            <View>
              ...
              <View
                style={tailwind(
                  'mt-2 flex-row justify-between items-center pb-2'
                )}
              >
